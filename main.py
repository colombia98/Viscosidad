...
# ALERTA DE FORMATO DECIMAL
st.error("🔴 **IMPORTANTE - FORMATO DE NÚMEROS:** \nPara los decimales debes utilizar **OBLIGATORIAMENTE EL PUNTO (.)**. No utilices la coma (,). \n✅ Correcto: `298.15` \n❌ Incorrecto: `298,15`")

st.sidebar.header("Configuración del Sistema")
fase = st.sidebar.selectbox("Fase de la Mezcla:", ["Mezcla Gaseosa (Cracking)"])
...
if menu == "Sustancias Puras":
    ...
    with st.expander("Ver Ecuación y Significado de Variables"):
        if modelo == "Chapman-Enskog":
            ...
            st.latex(r"\mu = \frac{40.785 F_c \sqrt{MT}}{V_c^{2/3} \Omega_v} \quad (T^* = 1.2593 \cdot T_r)")
        elif modelo == "DIPPR":
    ...
    with c2:
        ...
        if modelo == "Chapman-Enskog":
            sigma = st.number_input("Sigma (σ en Å):", format="%.4f")
            eps_k = st.number_input("Epsilon/k (ε/κ en K):", format="%.2f")
        elif modelo == "Chung et al.":
            tc = st.number_input("Temp. Crítica (Tc en K):", format="%.2f")
            vc = st.number_input("Vol. Crítico (Vc en cm³/mol):", format="%.2f")
            ...
            tc = st.number_input("Temp. Crítica (Tc en K):", format="%.2f")
            pc = st.number_input("Pres. Crítica (Pc en atm):", format="%.2f")
    
    with c3:
        if modelo == "Chung et al.":
            dipolo = st.number_input("Momento Dipolar (D):", value=0.0, format="%.4f")
            kappa = st.number_input("Factor de Asociación (κ):", value=0.0, format="%.4f")
        elif modelo == "DIPPR":
    ...
    if st.button("Ejecutar Cálculo"):
        try:
            # LÓGICA CORREGIDA SEGÚN TU EXCEL Y EL LIBRO DE POLING
            if modelo == "Chapman-Enskog":
                t_star = t_abs / eps_k
                mu_uP = (26.69 * np.sqrt(m_in * t_abs)) / (sigma**2 * omega_v(t_star))
                mu_c = mu_uP / 10  # de micropoises a uPa.s
                
            elif modelo == "Chung et al.":
                tr = t_abs / tc
                t_star_chung = 1.2593 * tr # <- EL FACTOR CORREGIDO!
                mu_r = 131.3 * dipolo / np.sqrt(vc * tc)
                fc = 1 - 0.2756 * w + 0.059035 * mu_r**4 + kappa
                mu_uP = (40.785 * fc * np.sqrt(m_in * t_abs)) / (vc**(2/3) * omega_v(t_star_chung))
                mu_c = mu_uP / 10  # de micropoises a uPa.s
                
            elif modelo == "DIPPR":
                mu_Pa_s = (a_dip * t_abs**b_dip) / (1 + c_dip/t_abs + d_dip/t_abs**2)
                mu_c = mu_Pa_s * 1e6 # de Pa.s a uPa.s
                
            elif modelo == "Stiel y Thodos":
                tr = t_abs / tc
                # Cálculo de N correcto en magnitud
                if tr > 1.5:
                ...
                mu_cP = N / xi # Resultado original en centipoises
                mu_c = mu_cP * 1000 # de cP a uPa.s
            ...
            st.error(f"Error técnico: Verifica que no hayas dejado variables en cero. Detalle: {e}")
    ...
    st.info("Ingresa la fracción molar (xi) y la viscosidad (μi) de cada componente calculada previamente. USA PUNTO PARA LOS DECIMALES.")
    ...
    if st.button("Calcular Viscosidad de Mezcla"):
        ...
        try:
            ...
            st.error(f"Error en el cálculo matricial. Verifica que las viscosidades y masas no sean 0. Detalle: {e}")
